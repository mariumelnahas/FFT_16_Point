module compare_files;
  ...
  initial begin
    ...
    fd_m = $fopen("Verification/rtl_outputs.txt", "r");
    fd_g = $fopen("Verification/golden_fft_outputs.txt", "r");

    if (fd_m == 0) begin
      $display("ERROR: Cannot open rtl_outputs.txt");
      $finish;
    end

    if (fd_g == 0) begin
      $display("ERROR: Cannot open golden_fft_outputs.txt");
      $finish;
    end
