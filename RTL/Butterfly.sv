module butterfly #(
    ...
    output wire signed [2*WIDTH-1:0module butterfly #(
    parameter integer STAGE = 1,   // FFT stage: 1, 2, 3, or 4
    parameter integer WIDTH = 16
)(
    // Packed complex inputs: {real[WIDTH-1:0], imag[WIDTH-1:0]}
    ...
    // -------------------------------------------------------------------
    // Elaboration-time guard: only stages 1-4 are defined
    // -------------------------------------------------------------------
    initial begin
        if (STAGE < 1 || STAGE > 4)
            $error("butterfly: STAGE must be 1-4, got %0d", STAGE);
    end

endmodule] diff_out    // top - bot
);
