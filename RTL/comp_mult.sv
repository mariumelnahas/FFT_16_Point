...
module complex_multiplier #(
    ...
    function automatic signed [15:0] round_shift(input signed [31:0] val, input integer shift);
        ...
        begin
            if (shift <= 0) begin
                // No fractional bits to drop (not expected for any stage
                // config in this design, but handle it exactly if it occurs).
                rounded = val <<< (-shift);
            end else begin
            ...
            if (rounded > 32'sd32767)
                round_shift = 16'sd32767;
            else if (rounded < -32'sd32768)
                round_shift = -16'sd32768;
            else
